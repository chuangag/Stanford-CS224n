...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    denomiator=np.array([np.apply_along_axis(lambda x: np.sqrt(x.T.dot(x)),1,x)]).T
    x=x/denomiator
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    yhat[target] -= 1.0
    yhat_minus_y=yhat
    grad = np.outer(yhat_minus_y, vhat)
    #print "yhat outer vhat: ",yhat.shape,"x",vhat.shape,"=",grad.shape
    gradPred = np.dot(outputVectors.T, yhat_minus_y)
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    v_center=inputVectors[current_index]

    for w in contextWords:
        w_idx=tokens[w]
        c_cost,c_grad_in,c_grad_out= word2vecCostAndGradient(v_center,w_idx,outputVectors,dataset)
        cost+=c_cost
        gradIn[current_index]+=c_grad_in
        gradOut+=c_grad_out
    ### END YOUR CODE
